...
export default function StudioPage({ channelScale, channelMode, channelStyle }: StudioPageProps = {}) {
  ...
  const playAll = useCallback(async () => {
    ...
    animRef.current = requestAnimationFrame(tickFn);
  }, [ensureTone, setupMetronome, tracks, currentTime, metronomeOn, bpm, duration, looping]);
  ...
  // ── Save to recordings (simple blob save) ──
  const saveToRecordings = useCallback(async () => {
    if (tracks.length === 0) return;
    setSavingToLibrary(true);
    try {
      // Find first track with audio to save
      const audioTrack = tracks.find(t => t.audioBlob);
      if (audioTrack && audioTrack.audioBlob) {
        const id = `rec-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
        const ws = wsRef.current[audioTrack.id];
        const dur = ws ? ws.getDuration() : 0;
        const meta = await idbSaveRecording(id, audioTrack.name, audioTrack.audioBlob, dur);
  ...
  return (
    <div className="flex flex-col overflow-hidden select-none" style={{ background: "#0a0a0a", fontFamily: "'Inter', system-ui, sans-serif", height: "calc(100vh - 90px)" }} dir="ltr">
      ...
      {/* ═══════════ TOP BAR: Device + BPM + Metronome + Record ═══════════ */}
      <div className="flex items-center h-12 px-3 sm:px-4 gap-2 sm:gap-3 border-b flex-shrink-0" style={{ background: "#111111", borderColor: "#1e1e1e" }}>
        {/* Mic device selector */}
        <div className="flex items-center gap-2">
          ...
          <select value={selectedInputDevice}
            ...
            className="bg-[#1a1a1a] border border-[#2a2a2a] rounded px-2 py-1 text-[10px] text-[#aaa] outline-none focus:border-[#D4A843] cursor-pointer max-w-[160px] truncate">
            {inputDevices.map((d) => (
        ...
        <div className="flex items-center gap-1">
          <input type="number" min={40} max={300} value={bpm}
            ...
            className="w-12 h-7 bg-transparent border border-[#2a2a2a] rounded text-[#D4A843] text-xs text-center font-mono focus:border-[#D4A843] outline-none hover:border-[#444] transition-colors" />
          <span className="text-[10px] text-[#444] font-medium">BPM</span>
        </div>
        ...
        <button onClick={() => setLooping(!looping)} title="Loop (C)"
          className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all cursor-pointer ${looping ? "text-[#D4A843]" : "text-[#555] hover:text-[#888]"}`}
          style={{ background: looping ? "#2a2418" : "transparent", border: looping ? "1px solid #D4A84355" : "1px solid transparent" }}>
        ...
        {isRec && (
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-[#C41E3A] animate-pulse" />
            <span className="font-mono text-xs text-[#C41E3A]">{fmtTime(recTime)}</span>
            {/* Live level */}
            <div className="w-16 h-2 bg-[#111] rounded overflow-hidden" style={{ border: "1px solid #1a1a1a" }}>
        ...
        {!isRec ? (
          <button onClick={startRec} title="Record (R)"
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all cursor-pointer hover:brightness-110"
            style={{ background: "#2a1015", border: "1px solid #C41E3A55" }}>
            <div className="w-3 h-3 rounded-full" style={{ background: "radial-gradient(circle at 40% 35%, #ff4466, #C41E3A)" }} />
            <span className="text-[10px] text-[#C41E3A] font-medium">Record</span>
          </button>
        ) : (
        ...
        <button onClick={saveToRecordings} disabled={tracks.length === 0 || savingToLibrary}
          className="text-[10px] font-medium px-3 py-1.5 rounded-md transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
          style={{
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3"
        ...
        {/* ─── TRANSPORT BAR ─── */}
        <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: "#111", border: "1px solid #1e1e1e" }}>
          <button onClick={rewindToStart} title="Rewind (Enter)"
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-[#888] group-hover:text-[#ccc] transition-colors">
          ...
          {!playing ? (
            <button onClick={playAll} title="Play (Space)" disabled={tracks.length === 0}
              className="w-9 h-9 rounded-lg flex items-center justify-center transition-all cursor-pointer group disabled:opacity-30"
              style={{ background: "#1a3a1a", border: "1px solid #33aa3366" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" className="ml-0.5 text-[#4ade80] group-hover:text-white transition-colors">
            ...
            <button onClick={stopAll} title="Pause (Space)"
              ...
              style={{ background: "#1a3a1a", border: "1px solid #22c55e", boxShadow: "0 0 8px rgba(34,197,94,0.2)" }}>
          ...
          {/* Time display */}
          <div className="px-3 py-1 rounded font-mono text-sm flex items-center min-w-[110px] justify-center relative overflow-hidden"
            style={{ background: "#050505", border: "1px solid #1a1a1a", boxShadow: "inset 0 1px 4px rgba(0,0,0,0.5)" }}>
            <span className="text-[#D4A84333] absolute tracking-[2px]" style={{ fontFamily: "'Courier New', monospace", fontSize: "14px" }}>88:88.8</span>
            <span className="text-[#D4A843] relative tracking-[2px]" style={{ fontFamily: "'Courier New', monospace", fontSize: "14px", textShadow: "0 0 8px rgba(212,168,67,0.4)" }}>{fmtTime(currentTime)}</span>
          </div>
          <span className="text-[10px] text-[#444] font-mono">/</span>
          <span className="text-[10px] text-[#555] font-mono">{fmtTime(duration)}</span>

          <div className="flex-1" />
          <span className="text-[9px] text-[#333] font-mono">{tracks.length} tracks</span>
          {looping && <span className="text-[8px] text-[#D4A843] bg-[#D4A84315] px-1.5 py-0.5 rounded">LOOP</span>}
        </div>

        {/* ─── TRACK CARDS ─── */}
        {tracks.map((tr) => (
          <div key={tr.id} className="rounded-lg overflow-hidden transition-all" style={{ background: "#111", border: `1px solid ${tr.color}33` }}>
            {/* Track header */}
            <div className="flex items-center gap-2 px-3 py-2" style={{ borderBottom: `1px solid ${tr.color}22` }}>
              <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: tr.color }} />
              ...
              {editingTrackName === tr.id ? (
                <input autoFocus defaultValue={tr.name}
                  ...
                  className="flex-1 bg-[#0a0a0a] border border-[#D4A843] rounded px-2 py-0.5 text-[11px] text-[#ccc] outline-none min-w-0"
                />
              ) : (
              ...
              <input type="range" min={0} max={100} value={tr.volume}
                ...
                className="w-20 h-[3px] cursor-pointer" style={{ accentColor: tr.color }} />
              <span className="text-[8px] text-[#555] font-mono w-6 text-right">{tr.volume}%</span>

              {/* Mute */}
              <button onClick={() => toggleMute(tr.id)}
                className={`text-[8px] font-bold w-6 h-6 rounded cursor-pointer flex items-center justify-center transition-all ${tr.muted ? "bg-[#3b82f6] text-white" : "border border-[#2a2a2a] text-[#555] hover:border-[#444] hover:text-[#888]"}`}>
                M
              </button>

              {/* Drum grid toggle */}
              {tr.type === "drum" && (
                <button onClick={() => setExpandedDrumTrackId(expandedDrumTrackId === tr.id ? null : tr.id)}
                  className={`text-[8px] px-2 h-6 rounded cursor-pointer transition-all ${expandedDrumTrackId === tr.id ? "bg-[#D4A84320] text-[#D4A843] border border-[#D4A84340]" : "border border-[#2a2a2a] text-[#555] hover:border-[#444]"}`}>
                  Grid
                </button>
              )}
              ...
              <button onClick={() => deleteTrack(tr.id)}
                className="w-6 h-6 rounded flex items-center justify-center text-[#444] hover:text-[#ef4444] hover:bg-[#ef444410] transition-all cursor-pointer">
            ...
            {/* Waveform display (non-drum tracks) */}
            {tr.type !== "drum" && (
              <div ref={(el) => { if (el) trackContainersRef.current[tr.id] = el; }}
                className="min-h-[48px]" style={{ opacity: tr.muted ? 0.3 : 1 }} />
            )}

            {/* Drum grid (inline, collapsible) */}
            {tr.type === "drum" && expandedDrumTrackId === tr.id && tr.drumPattern && (
              <div className="p-3 overflow-x-auto" style={{ background: "#0c0c0c" }}>
                <div className="flex items-center gap-3 mb-2">
                  <button
                    onClick={() => {
                      if (drumPlaying) stopDrumPlayback();
                      else if (tr.drumPattern) startDrumPlayback(tr.drumPattern, tr.id);
                    }}
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[10px] font-medium cursor-pointer transition-colors ${drumPlaying ? "bg-[#ef4444] text-white" : "bg-[#D4A843] text-[#111]"}`}>
                    {drumPlaying ? "Stop" : "Play"}
                  </button>
                  <span className="text-[9px] text-[#555]">{bpm} BPM</span>
                  <button onClick={() => {
                    setTracks((prev) => prev.map((t) => t.id === tr.id ? { ...t, drumPattern: createEmptyDrumPattern() } : t));
                  ...
                  <select title="Drum preset"
                    onChange={(e) => {
                      ...
                      if (idx >= 0 && idx < DRUM_PRESETS.length) {
                        setTracks((prev) => prev.map((t) => t.id === tr.id ? { ...t, drumPattern: DRUM_PRESETS[idx].pattern.map(r => [...r]) } : t));
                      }
                      e.target.value = "";
                    }}
                    defaultValue=""
                ...
                <div className="inline-block" style={{ minWidth: "fit-content" }}>
                  <div className="flex items-center mb-1" style={{ paddingLeft: 72 }}>
                    {Array.from({ length: DRUM_STEPS }, (_, i) => (
                      <div key={i} className={`w-6 h-4 flex items-center justify-center text-[7px] font-mono ${drumStep === i ? "text-[#D4A843]" : i % 4 === 0 ? "text-[#555]" : "text-[#333]"}`}>{i + 1}</div>
                    ))}
                  </div>
                  {DRUM_INSTRUMENTS.map((instr, instrIdx) => (
                    <div key={instr.name} className="flex items-center mb-0.5">
                      ...
                      {tr.drumPattern![instrIdx].map((on, stepIdx) => (
                        <button key={stepIdx}
                          onClick={() => toggleDrumCell(tr.id, instrIdx, stepIdx)}
                          className={`w-6 h-6 rounded-sm cursor-pointer transition-all border ${on ? "border-[#D4A84366]" : stepIdx % 4 === 0 ? "border-[#2a2a2a] hover:border-[#444]" : "border-[#1e1e1e] hover:border-[#333]"}`}
                          style={{
                            background: on
        ...
        {tracks.length === 0 && !isRec && (
          <div className="flex items-center justify-center py-16">
            <div className="border-2 border-dashed rounded-2xl px-16 py-12 text-center" style={{ borderColor: "#333", maxWidth: "420px" }}>
              ...
              <div className="text-[#333] text-[11px]">or use <span className="text-[#C41E3A]">Record</span> / <span className="text-[#888]">Add Track</span> to get started</div>
            </div>
          </div>
        )}
        ...
        <div className="relative">
          <div className="flex items-center gap-2 flex-wrap">
            <button onClick={() => setShowAddTrackMenu(!showAddTrackMenu)}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-[10px] text-[#888] hover:text-[#D4A843] transition-colors cursor-pointer"
            ...
            <button onClick={startRec}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded text-[9px] text-[#777] hover:text-[#C41E3A] transition-colors cursor-pointer border border-[#1e1e1e] hover:border-[#C41E3A33]">
              {"\uD83C\uDF99"} Record
            </button>
            <button onClick={addDrumTrack}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded text-[9px] text-[#777] hover:text-[#D4A843] transition-colors cursor-pointer border border-[#1e1e1e] hover:border-[#D4A84333]">
              {"\uD83E\uDD41"} Drums
            </button>
            <button onClick={() => setShowSunoPanel(!showSunoPanel)}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded text-[9px] text-[#777] hover:text-[#D4A843] transition-colors cursor-pointer border border-[#1e1e1e] hover:border-[#D4A84333]">
              {"\uD83C\uDFB5"} AI Backing
            </button>
          ...
          {showAddTrackMenu && (
            <div className="absolute top-12 left-0 z-50 bg-[#1e1e1e] border border-[#333] rounded-lg shadow-2xl py-1 min-w-[200px]"
              ...
              <button onClick={() => { startRec(); setShowAddTrackMenu(false); }}
                ...
                <div><div className="font-medium">Record</div><div className="text-[9px] text-[#666]">Mic / Audio Interface</div></div>
              </button>
              <button onClick={() => { fileRef.current?.click(); setShowAddTrackMenu(false); }}
                className="w-full text-left text-[11px] text-[#ccc] hover:bg-[#2a2a2a] px-4 py-2.5 cursor-pointer transition-colors flex items-center gap-3">
                <span className="text-[16px]">{"\uD83D\uDCC1"}</span>
                <div><div className="font-medium">Import File</div><div className="text-[9px] text-[#666]">WAV, MP3, OGG, FLAC</div></div>
              </button>
              <div className="border-t border-[#2a2a2a] my-0.5" />
              <button onClick={addDrumTrack}
                className="w-full text-left text-[11px] text-[#ccc] hover:bg-[#2a2a2a] px-4 py-2.5 cursor-pointer transition-colors flex items-center gap-3">
                <span className="text-[16px]">{"\uD83E\uDD41"}</span>
                <div><div className="font-medium">Drum Machine</div><div className="text-[9px] text-[#666]">Step sequencer pattern</div></div>
              </button>
              <button onClick={() => { setShowSunoPanel(true); setShowAddTrackMenu(false); }}
                className="w-full text-left text-[11px] text-[#ccc] hover:bg-[#2a2a2a] px-4 py-2.5 cursor-pointer transition-colors flex items-center gap-3">
                <span className="text-[16px]">{"\uD83C\uDFB5"}</span>
        ...
        {showSunoPanel && (
          <div className="rounded-lg overflow-hidden" style={{ background: "#111", border: "1px solid #D4A84333" }}>
            <div className="flex items-center justify-between px-3 py-2" style={{ background: "#151512", borderBottom: "1px solid #222" }}>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-[#ccc] font-medium">Suno AI</span>
              </div>
              <div className="flex items-center gap-2">
                {sunoCredits !== null && (
                ...
                <button onClick={() => setShowSunoPanel(false)} className="text-[#444] hover:text-[#888] cursor-pointer transition-colors">
            ...
            <div className="flex" style={{ borderBottom: "1px solid #222" }}>
              {(["generate", "library", "credits"] as const).map((tab) => (
                <button key={tab} onClick={() => setSunoTab(tab)}
                  className="flex-1 text-[10px] py-2 font-medium transition-all cursor-pointer"
                  style={{ color: sunoTab === tab ? "#D4A843" : "#555", background: sunoTab === tab ? "#1e1e1e" : "transparent", borderBottom: sunoTab === tab ? "2px solid #D4A843" : "2px solid transparent" }}>
              ...
              {sunoTab === "generate" && (
                <div className="space-y-2">
                  ...
                  {!sunoCustomPrompt ? (
                      ...
                      <div className="flex gap-2">
                        <label className="flex-1">
                          ...
                          <select value={sunoScale} onChange={(e) => setSunoScale(e.target.value)}
                            className="w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded px-2 py-1 text-[10px] text-[#aaa] outline-none focus:border-[#D4A843] cursor-pointer">
                      ...
                      <div className="flex gap-2">
                        ...
                        <label className="w-24">
                          ...
                          <input type="number" value={sunoBpm} min={40} max={300}
                            ...
                            className="w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded px-2 py-1 text-[10px] text-[#D4A843] outline-none focus:border-[#D4A843] font-mono" />
                        </label>
                      </div>
                    </>
                  ) : (
                    <div>
                      ...
                      <textarea value={sunoPromptText}
                        ...
                        rows={3}
                        className="w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded px-2 py-1.5 text-[10px] text-[#aaa] outline-none focus:border-[#D4A843] resize-none placeholder:text-[#333]" />
                  ...
                  {!sunoConfirm ? (
                    ...
                    <div className="rounded p-2.5 space-y-2" style={{ background: "#0e0e0e", border: "1px solid #D4A84320" }}>
                      ...
                      <div className="flex gap-1.5">
                        ...
                        <button onClick={() => setSunoConfirm(false)}
                          className="flex-1 text-[10px] py-1.5 rounded bg-[#222] text-[#888] hover:bg-[#333] cursor-pointer transition-all">Cancel</button>
                      </div>
                    </div>
                  )}
                  ...
                  {sunoGeneratedTracks.length > 0 && (
                    <div className="space-y-1.5">
                      ...
                      {sunoGeneratedTracks.map((t) => (
                        <div key={t.id} className="flex items-center gap-2 rounded p-2" style={{ background: "#0e0e0e", border: "1px solid #1e1e1e" }}>
              ...
              {sunoTab === "library" && (
                <div className="space-y-2">
                  <input value={sunoLibSearch} onChange={(e) => setSunoLibSearch(e.target.value)}
                    ...
                    className="w-full bg-[#0e0e0e] border border-[#2a2a2a] rounded px-2 py-1.5 text-[10px] text-[#aaa] outline-none focus:border-[#D4A843] placeholder:text-[#333]" />
                  <div className="space-y-1 max-h-[300px] overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#333 transparent" }}>
                    {(() => {
                      ...
                      return sorted.map((t) => (
                        <div key={t.id} className="rounded p-2 group hover:bg-[#1a1a1a] transition-colors" style={{ background: "#0e0e0e", border: "1px solid #1a1a1a" }}>
                          <div className="flex items-center gap-2">
                            <button onClick={() => sunoPlayTrack(t.audioUrl, t.id)}
        ...
        <div className="rounded-lg overflow-hidden" style={{ background: "#111", border: "1px solid #1e1e1e" }}>
          <div className="flex items-center justify-between px-3 py-2" style={{ background: "#141414", borderBottom: "1px solid #1e1e1e" }}>
            ...
            <input value={recSearchQuery} onChange={(e) => setRecSearchQuery(e.target.value)}
              placeholder="Search..."
              className="bg-[#0e0e0e] border border-[#2a2a2a] rounded px-2 py-1 text-[9px] text-[#aaa] outline-none focus:border-[#D4A84333] w-32 placeholder:text-[#333]" />
          </div>
          ...
          {filteredRecordings.length === 0 ? (
            ...
            <div className="divide-y divide-[#1a1a1a] max-h-[300px] overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "#333 transparent" }}>
              {filteredRecordings.map((rec) => {
                ...
                return (
                  <div key={rec.id} className="flex items-center gap-2 px-3 py-2 hover:bg-[#1a1a1a] transition-colors group">
                    {/* Play/pause */}
