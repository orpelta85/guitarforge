  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="mb-4 rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.05)", background: "#111114" }}>
        <button onClick={() => setJamOpen(p => !p)} className="panel-header flex items-center gap-5 w-full cursor-pointer bg-transparent border-0 text-left px-7 py-8 hover:bg-white/[0.02] transition-colors">
          ...
          <span className="flex-1 font-heading text-[22px] font-semibold text-[#e5e5e5]">Quick Jam</span>
          <span className="font-readout text-[14px] text-[#888]">
            {!jamOpen && `${jamKey} ${jamScale} \u00B7 ${jamStyle}`}
          </span>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2" className={`transition-transform ${jamOpen ? "rotate-180" : ""}`}>
            <path d="M6 9l6 6 6-6" />
          </svg>
        </button>
        {jamOpen && (
