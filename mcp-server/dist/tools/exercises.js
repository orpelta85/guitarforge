...
export function registerExerciseTools(server) {
    // gf_list_exercises
    server.registerTool("gf_list_exercises", {
  ...
  - hasTab (boolean, optional): Filter exercises that have AlphaTex notation`,
        ...
        try {
            ...
            if (search) {
                ...
                exercises = exercises.filter(e => e.n.toLowerCase().includes(q) ||
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error listing exercises: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
    // gf_get_exercise
    server.registerTool("gf_get_exercise", {
...
Args:
  - id (number): The exercise ID`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error getting exercise: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
    // gf_add_exercise
    server.registerTool("gf_add_exercise", {
        title: "Add Exercise",
...
Returns: { success, id, message }`,
        inputSchema: {
            category: z.enum(CATS).describe("Exercise category"),
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error adding exercise: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
    // gf_write_alphatex
    server.registerTool("gf_write_alphatex", {
...
Returns: { success, message }`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error writing AlphaTex: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
}
