...
export function registerSongTools(server) {
    // gf_list_songs
    server.registerTool("gf_list_songs", {
        title: "List Songs",
...
Returns: Array of song entries with all fields.`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error listing songs: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
    // gf_add_song
    server.registerTool("gf_add_song", {
        title: "Add Song",
...
Returns: { success, id, message }`,
        ...
        try {
            ...
            return {
                isError: true,
                content: [{ type: "text", text: `Error adding song: ${error instanceof Error ? error.message : String(error)}` }],
            };
        }
    });
}
